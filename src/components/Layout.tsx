...
import { Menu, X, ChevronDown, Mail, Phone, MapPin, Linkedin, Instagram } from 'lucide-react';
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-sm py-4' : 'bg-white/80 backdrop-blur-md py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <Link to="/" className="flex items-center space-x-3">
          <img 
            src="https://enlizboqeqximxogepig.supabase.co/storage/v1/object/sign/Luismkt/Gemini_Generated_Image_urtay5urtay5urta.png?token=eyJraWQiOiJzdG9yYWdlLXVybC1zaWduaW5nLWtleV83NWNhYTVkMy00MTc0LTQ4MDMtYThkMy0xODkxYzMzYTRmOGIiLCJhbGciOiJIUzI1NiJ9.eyJ1cmwiOiJMdWlzbWt0L0dlbWluaV9HZW5lcmF0ZWRfSW1hZ2VfdXJ0YXk1dXJ0YXk1dXJ0YS5wbmciLCJpYXQiOjE3NzM0OTExNjEsImV4cCI6MTgwNTAyNzE2MX0.gxerRJ-O7eJmNepKCC_ts5ggMRXaMjT1BAcYBuyDBSA" 
            alt="LUIS MKT Logo" 
    ...
    <footer className="bg-brand-navy text-white pt-20 pb-10">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mb-20">
        ...
        <div>
          ...
          <ul className="space-y-4 text-sm text-gray-300">
            <li className="flex items-center gap-3"><Mail size={16} className="text-brand-red" /> info@luismkt.com</li>
            <li className="flex items-center gap-3"><Phone size={16} className="text-brand-red" /> +34 600 000 000</li>
            <li className="flex items-center gap-3"><MapPin size={16} className="text-brand-red" /> Madrid, España</li>
          </ul>
        </div>
